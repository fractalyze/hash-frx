...
class Blake2bMarkerTest(absltest.TestCase):
    def test_routing_is_the_pin_and_the_backend(self) -> None:
        ...
        self.assertTrue(blake2b._DEDICATED_EMITTER_AVAILABLE)
        self.assertEqual(blake2b._EMITTER_BACKENDS, ("cpu", "gpu"))
        # One instance suffices: the path reads the two flags alone, and the
        # size sweeps elsewhere already construct every length.
        self.assertIs(
            Blake2b().fusion_path, FusionPath.from_routing(_HAS_BLAKE2B_EMITTER)
        )
    ...
    def test_digest_emits_one_composite_with_the_digest_name(self) -> None:
        ...
        # The wire now carries the shared operation name, with BLAKE2b named by
        # the `primitive` attribute; the list length is the composite count.
        msg = np.zeros((4, 127), dtype=np.uint8)
        self.assertEqual(
            emitted_composites(blake2b.digest, msg), [markers.BYTES_DIGEST_MARKER]
        )
    ...
    def test_the_digest_compiles_to_a_custom_fusion(self) -> None:
        ...
        if not _HAS_BLAKE2B_EMITTER:
            self.skipTest(f"no BLAKE2b emitter on {frx.default_backend()}")
        # (4, 129) is the aval `Blake2bTracedTest` already drives through
        # `frx.jit(blake2b.digest)`, so the two share one compile — the only
        # aval here that does, since the differential sweep calls `digest`
        # eagerly and so compiles a different function object.
        msg = fnp.asarray(np.zeros((4, 129), dtype=np.uint8))
        assert_marker_recognized(
            self, "blake2b", blake2b.digest, msg, envelope_key="bytes_digest"
        )
...
class Blake2bByteHashTest(absltest.TestCase):
    ...
    def test_fusion_paths_pin_the_substrate(self) -> None:
        ...
        device = Blake2b()
        self.assertIs(device.fusion_path, FusionPath.from_routing(_HAS_BLAKE2B_EMITTER))
    ...
    def test_digest_shape_and_dtype(self) -> None:
        out = np.asarray(Blake2b(20).digest(np.zeros((4, 1), dtype=np.uint8)))

...
class FinalizeBoundsTest(parameterized.TestCase):
    ...
    128-bit count in bits, and a byte count near 2^31 has a bit length near
    2^34 — held to `int.to_bytes` across the wrap the one-word form had.
    """

    @parameterized.parameters((127, 112), (0, 112), (111, 1), (127, 1), (64, 48))
    def test_widest_extras_still_match_hashlib(self, pending: int, width: int) -> None:
        state = sha512.sha512_stream_absorb(
            sha512.sha512_stream_init(), fnp.zeros(pending, dtype=fnp.uint8)
        )
        got = np.asarray(
            sha512.sha512_stream_finalize(state, fnp.zeros((1, width), dtype=fnp.uint8))
        )[0]
        want = hashlib.sha512(bytes(pending + width)).digest()
        self.assertEqual(got.tobytes(), want)

    def test_extras_past_the_layout_are_rejected(self) -> None:
        state = sha512.sha512_stream_init()
